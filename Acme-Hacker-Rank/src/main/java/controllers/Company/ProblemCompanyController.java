...
public class ProblemCompanyController extends AbstractController {
	...
	public ModelAndView list(final RedirectAttributes redirectAttrs) {
		...
		try {
			...
			final Integer companyId = this.companyService.findCompanyByUseraccount(LoginService.getPrincipal()).getId();
			Assert.notNull(this.companyService.findOne(companyId));

			result = new ModelAndView("problem/list");
			final Collection<Problem> problems = this.problemService.findByCompanyId(companyId);

			result.addObject("problems", problems);
			result.addObject("requestURI", "problem/company/list.do?companyId=" + companyId);
	...
	public ModelAndView save(@Valid final ProblemForm problemForm, final BindingResult binding) {
		ModelAndView result;

		if (binding.hasErrors())
			result = this.createModelAndView(problemForm, "problem.commit.error");
		else
			try {
	...
	public ModelAndView edit(final int problemId, final RedirectAttributes redirectAttrs) {
		ModelAndView result;
		Problem problem;
		final Company b = this.companyService.findCompanyByUseraccountId(LoginService.getPrincipal().getId());
		try {
			problem = this.problemService.findOne(problemId);
			Assert.notNull(problem);
			Assert.isTrue(this.problemService.findOne(problemId).getPosition().getCompany().equals(b));
			...
			if (this.problemService.findOne(problemId) == null)
				redirectAttrs.addFlashAttribute("message", "problem.error.unexist");
			else if (this.problemService.findOne(problemId).isDraftmode() == false)
				redirectAttrs.addFlashAttribute("message", "problem.error.notDraftMode");
			else if (!this.problemService.findOne(problemId).getPosition().getCompany().equals(b))
				redirectAttrs.addFlashAttribute("message", "problem.error.notFromActor");
		}
		return result;
	}

	@RequestMapping(value = "/edit", method = RequestMethod.POST, params = "save")
	public ModelAndView save2(@Valid final ProblemForm problemForm, final BindingResult binding) {
		ModelAndView result;
		final Company b = this.companyService.findCompanyByUseraccountId(LoginService.getPrincipal().getId());
		if (binding.hasErrors())
			...
			try {
				...
				final Problem problem = this.problemService.findOne(problemForm.getId());
	...
	public ModelAndView delete(@Valid final ProblemForm problemForm, final BindingResult binding) {
		ModelAndView result;
		final Company b = this.companyService.findCompanyByUseraccountId(LoginService.getPrincipal().getId());
		if (binding.hasErrors())
			result = this.editModelAndView(problemForm, "problem.commit.error");
			...
			try {
				...
				this.problemService.delete(this.problemService.findOne(problemForm.getId()));
	...
	public ModelAndView show(final int problemId, final RedirectAttributes redirectAttrs) {
		ModelAndView result;
		...
		try {
			problem = this.problemService.findOne(problemId);
			Assert.notNull(problem);
			Assert.isTrue(problem.getPosition().getCompany().getId() == b.getId());
	...
	protected ModelAndView createModelAndView(final ProblemForm problemForm, final String message) {
	...
	protected ModelAndView editModelAndView(final ProblemForm problemForm, final String message) {
		final ModelAndView result;

		result = new ModelAndView("problem/edit");

		result.addObject("message", message);
		result.addObject("requestURI", "problem/company/edit.do?problemId=" + problemForm.getId());
		result.addObject("problemForm", problemForm);
		result.addObject("isRead", false);
		result.addObject("banner", this.configurationService.findAll().iterator().next().getBanner());
		result.addObject("systemName", this.configurationService.findAll().iterator().next().getSystemName());
	...
	protected ModelAndView ShowModelAndView(final ProblemForm problemForm, final String message) {
		final ModelAndView result;

		result = new ModelAndView("problem/show");
		result.addObject("message", message);
		result.addObject("requestURI", "problem/company/show.do?problemId=" + problemForm.getId());
		result.addObject("problemForm", problemForm);
